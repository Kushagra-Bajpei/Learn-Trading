      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          <motion.div ref={ref} initial={{ opacity: 0, x: -50 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8 }}>
            ...
            <p className="text-[#a8c5b2] text-lg leading-relaxed mb-8">
            ...
            <div className="grid grid-cols-2 gap-4 mb-8">
              {[
                ...
                <div key={i} className="flex items-start gap-3 bg-[#0f1a14] border border-[#1e3028] rounded-xl p-4">
                  <div className="text-[#00ff88] mt-0.5">{item.icon}</div>
                  <div>
                    <p className="text-white text-sm font-semibold">{item.title}</p>
                    <p className="text-[#6b8c74] text-xs mt-0.5">{item.sub}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div ref={panelRef} className="opacity-0 space-y-4">
              ...
              <div className="space-y-3">
                {sessions.map((s, i) => (
                  <motion.div key={i} initial={{ opacity: 0, x: 20 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ delay: 0.3 + i * 0.1 }}
                    className="flex items-center gap-4 bg-[#0a0f0d] border border-[#1e3028] rounded-xl p-4 hover:border-[#00ff88]/30 transition-colors cursor-pointer group">
                    <div className="text-center bg-[#00ff88]/10 rounded-lg px-3 py-2 shrink-0">
                      <p className="text-[#00ff88] font-bold text-xs">{s.day.slice(0, 3).toUpperCase()}</p>
                      <Calendar size={14} className="text-[#00ff88] mx-auto mt-1" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-white text-sm font-semibold truncate">{s.topic}</p>
                      <p className="text-[#6b8c74] text-xs mt-0.5">{s.time}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-[#ff4d4d] text-xs font-bold">{s.seats} seats left</p>
                      <button className="text-[#00ff88] text-xs mt-1 group-hover:underline">Register →</button>
                    </div>
                  </motion.div>
                ))}
              </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {[{ val: '200+', label: 'Sessions Done' }, { val: '₹2.4Cr', label: 'Student Profits' }, { val: '94%', label: 'Satisfaction' }].map((s, i) => (
                <div key={i} className="gradient-card border border-[#1e3028] rounded-xl p-4 text-center">
                  <p className="text-[#00ff88] font-display font-bold text-2xl">{s.val}</p>
